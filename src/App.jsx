...
const C = {
  ...
  orange: "#F97316",
  yellow: "#FBBF24",
  green: "#10B981",import React, { useState, useEffect, useRef, useCallback } from "react";
import { supabase } from "./supabaseClient";
import {
...
function DrawScreen({ me, onSaved }) {
  ...
  const start = (e) => {
    ...
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.arc(lastPoint.current.x, lastPoint.current.y, size / 2, 0, Math.PI * 2);
    ctx.fill();
  };
  const move = (e) => {
    if (!drawing.current) return;
  ...
  return (
    <div style={{ padding: "14px 14px 100px", fontFamily: "Fredoka, sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
        {BRUSH_COLORS.map((c) => (
          <button key={c.hex} onClick={() => { setColor(c.hex); setEraser(false); }} style={{ width: 32, height: 32, borderRadius: "50%", background: c.hex, border: !eraser && color === c.hex ? "3px solid #3B2A55" : "3px solid white", boxShadow: "0 1px 4px rgba(0,0,0,0.2)", cursor: "pointer" }} />
        ))}
        <button onClick={() => setEraser(true)} style={{ width: 32, height: 32, borderRadius: "50%", background: "white", fontSize: 15, border: eraser ? `3px solid ${C.purple}` : "3px solid #EDE4FB", cursor: "pointer" }}>🧽</button>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
        <span style={{ fontSize: 13, color: "#8B7AA8" }}>Brush size</span>
        <input type="range" min={2} max={40} value={size} onChange={(e) => setSize(Number(e.target.value))} style={{ flex: 1 }} />
        <div style={{ width: size + 6, height: size + 6, borderRadius: "50%", background: eraser ? "#ccc" : color }} />
      </div>
...
// GALLERY SCREEN
// ============================================================
function GalleryScreen({ me, refreshKey }) {
  const [drawings, setDrawings] = useState(null);
  const [viewing, setViewing] = useState(null);
  ...
  return (
    <div style={{ padding: "14px 14px 100px", fontFamily: "Fredoka, sans-serif" }}>
      ...
      {viewing && (
        <div onClick={() => setViewing(null)} style={{ position: "fixed", inset: 0, background: "rgba(59,42,85,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: 20, padding: 16, maxWidth: 360, width: "100%" }}>
            <img src={viewing.data_url} alt={viewing.title} style={{ width: "100%", borderRadius: 14 }} />
            <div style={{ fontWeight: 700, color: C.ink, marginTop: 10 }}>{viewing.title}</div>
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button onClick={() => setViewing(null)} style={btnGhost}>Close</button>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: C.bg }}>
      ...
      {screen === "draw" && <DrawScreen me={me} onSaved={() => setRefreshKey((k) => k + 1)} />}
      {screen === "gallery" && <GalleryScreen me={me} refreshKey={refreshKey} />}
      {screen === "friends" && <FriendsScreen me={me} users={users} />}
      {screen === "feed" && <FeedScreen me={me} users={users} />}
